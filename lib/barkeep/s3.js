...
s3.syncDirectory = function (options, callback) {
    ...
    s3.getBucketObjects(options.prefix || '', function (err, bucketContents) {
        ...
        s3.changedFiles(updateFiles, bucketContents, options, function (filesToUpdate) {         
...
// ## putFile
// Put a file on the filesystem in S3.
s3.putFile = function (localFile, key, options, callback) {
    ...
    fs.readFile(localFile, function (err, dataBuffer) {
        ...
        var putOptions = {
            ...
            Body: dataBuffer,
            ContentType: mime.lookup(options.enableCompression === true ? localFile.replace('.gz', '') : localFile) 
        }
        ...
        //callback(null, putOptions);
        s3.client.PutObject(putOptions, function (err, data) {
            if (err) {
                return callback(err, null);
            }
